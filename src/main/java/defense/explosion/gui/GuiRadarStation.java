...
import defense.core.Rectangle;
import defense.core.Vector2;
import defense.core.gui.GuiBase;
...
public class GuiRadarStation extends GuiBase
{
    ...
    {
        this.fontRendererObj.drawString("\u00a77" + tileEntity.getInventoryName(), this.xSize / 2 - 30, 6, 4210752);

        this.fontRendererObj.drawString(LangUtils.localize("gui.radar.coords"), 155, 18, 4210752);
        this.fontRendererObj.drawString(LangUtils.localize("gui.misc.x") + " " + (int) Math.round(mouseOverCoords.x) + " " + LangUtils.localize("gui.misc.z") + " " + (int) Math.round(mouseOverCoords.y), 155, 30, 4210752);

        this.fontRendererObj.drawString("\u00a76" + this.info, 155, 42, 4210752);
        this.fontRendererObj.drawString("\u00a74" + this.info2, 155, 54, 4210752);

        this.fontRendererObj.drawString(LangUtils.localize("gui.radar.zoneSafe"), 152, 70, 4210752);
        this.textFieldSafetyZone.drawTextBox();
        this.fontRendererObj.drawString(LangUtils.localize("gui.radar.zoneAlarm"), 150, 85, 4210752);
        this.textFieldAlarmRange.drawTextBox();

        this.fontRendererObj.drawString(LangUtils.localize("gui.misc.freq"), 155, 100, 4210752);
        ...
        if (this.tileEntity.getEnergy() >= tileEntity.ENERGY_USAGE)
        {
            color = "\u00a72";
            status = LangUtils.localize("gui.radar.on");
        }
        else
        {
            status = LangUtils.localize("gui.radar.nopower");
        }
    ...
    public void keyTyped(char par1, int par2)
    {
        ...
        try
        {
            ...
            data.add(2);
            data.add(tileEntity.safetyRange);
        ...
        try
        {
            ...
            data.add(3);
            data.add(tileEntity.alarmRange);
        ...
        if (this.tileEntity.getEnergy() >= tileEntity.ENERGY_USAGE)
        {
            ...
            for (TileEntity jiQi : this.tileEntity.detectedTiles)
            {
                ...
                if (new Rectangle(minPosition, maxPosition).isIn(this.mousePosition))
                {
                    ...
                    {
                        if (jiQi.getBlockType() instanceof BlockMachine)
                        {
