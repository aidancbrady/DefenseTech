...
public class CommandICBM extends CommandBase
{
    ...
    public void processCommand(ICommandSender sender, String[] args)
    {
        try
        {
            ...
            {
                sender.addChatMessage(new ChatComponentText("/ICBM help"));
                sender.addChatMessage(new ChatComponentText("/ICBM lag <radius>"));
                sender.addChatMessage(new ChatComponentText("/ICBM remove <All/Missile/Explosion> <radius>"));
                sender.addChatMessage(new ChatComponentText("/ICBM emp <radius>"));
                return;
            }
            else if (args.length >= 2 && args[0].equalsIgnoreCase("lag"))
            {
                ...
                if (radius > 0)
                {
                    ...
                    sender.addChatMessage(new ChatComponentText("Removed all ICBM lag sources within " + radius + " radius."));
                    return;
                }
                else
                {
            ...
            else if (args.length >= 3 && args[0].equalsIgnoreCase("remove"))
            {
                ...
                if (radius > 0)
                {
                    ...
                    sender.addChatMessage(new ChatComponentText("Removed all ICBM " + str + " within " + radius + " radius."));
                    return;
                }
                else
                {
