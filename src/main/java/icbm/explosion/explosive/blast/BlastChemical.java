...
import icbm.explosion.potion.CustomPotionEffect;
...
public class BlastChemical extends Blast
{
    ...
    public void doPreExplode()
    {
        ...
        {
            this.worldObj.playSoundEffect(this.position.xPos, this.position.yPos, this.position.zPos, Reference.PREFIX + "debilitation", 4.0F, (1.0F + (worldObj.rand.nextFloat() - worldObj.rand.nextFloat()) * 0.2F) * 0.7F);
        }
    }
    ...
    public void doExplode()
    {
        ...
        if (this.worldObj.isRemote)
        {
            for (int i = 0; i < 200; i++)
            {
                ...
                diDian.xPos = Math.random() * radius / 2 - radius / 4;
                diDian.yPos = Math.random() * radius / 2 - radius / 4;
                diDian.zPos = Math.random() * radius / 2 - radius / 4;
        ...
        if (this.playShortSoundFX)
        {
            worldObj.playSoundEffect(position.xPos + 0.5D, position.yPos + 0.5D, position.zPos + 0.5D, Reference.PREFIX + "gasleak", 4.0F, (1.0F + (worldObj.rand.nextFloat() - worldObj.rand.nextFloat()) * 0.2F) * 1F);
        }
