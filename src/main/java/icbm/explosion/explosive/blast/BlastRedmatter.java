...
import icbm.api.IExplosiveIgnore;
        ...
        if (this.worldObj.isRemote)
        {
            if (entity instanceof EntityFlyingBlock)
            {
                if (ICBMExplosion.proxy.getParticleSetting() == 0)
                {
                    ...
                    {
                        ICBMExplosion.proxy.spawnParticle("digging", this.worldObj, new Pos3D(entity), -xDifference, -yDifference + 10, -zDifference, Block.getIdFromBlock(((EntityFlyingBlock) entity).block), 0, ((EntityFlyingBlock) entity).metadata, 2, 1);
